...
sub find_item { # find item for argument player
    ...
    if ($level > int($rps{$u}{item}{$type})) {
        chanmsg($rps{$u}{nick}." found a level $level $type! Their current $type is only ".
               "level ".int($rps{$u}{item}{$type}).", so it seems Luck is ".
               "with them!");
        $rps{$u}{item}{$type} = $level;
    }
    else {
        chanmsg($rps{$u}{nick}." found a level $level $type. Their current $type is level ".
               int($rps{$u}{item}{$type}).", so it seems Luck is against them. ".
               "they toss the $type.");
    }
}
...
sub daemonize() {
    ...
    $SIG{CHLD} = sub { };
    #open(STDIN,'/dev/null') || print debug("Cannot read /dev/null: $!",1);
    #open(STDOUT,'>/dev/null') || print debug("Cannot write to /dev/null: $!",1);
    #open(STDERR,'>/dev/null') || print debug("Cannot write to /dev/null: $!",1);
